...
final class MethodToRouteAnnotationFixer extends AbstractFixer
{
    ...
    public function getDefinition()
    {
        return new FixerDefinition(
            'Replace @Method annotation by @Route(..., methods={}) and replace 
            Sensio\Bundle\FrameworkExtraBundle\Configuration\Route by Symfony\Component\Routing\Annotation\Route',
            [
                new CodeSample(
                    '<?php 
/**
...
'
    ...
    {
        return $tokens->isAllTokenKindsFound([\T_DOC_COMMENT, \T_USE]);
    }
    ...
    public function applyFix(\SplFileInfo $file, Tokens $tokens)
    {
        foreach ($tokens as $index => $token) {
            if (!$token->isGivenKind([\T_DOC_COMMENT, \T_USE])) {
                continue;
            }
